...
local element2 = cv.CreateStructuringElementEx(8, 8, 1, 1, cv.CV_SHAPE_RECT)         
while (true) do
    ...
    if capture then
       ...
       -- print (fps)
        ...
        if image then
            ...
            local pixel_count, contornos = countPixels(image, 0, 179, 60, 200, 0, 25);
            --print("Cantidad de pixeles: ", pixel_count);
            
          
             
            --cv.Threshold(contornos, contornos, 100, 255, cv.CV_THRESH_BINARY)
            cv.Erode(contornos,contornos, element, 1)
            cv.Dilate(contornos,contornos, element2, 3)
